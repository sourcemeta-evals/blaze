...
TEST(Output_standard_position, flag_format_no_position) {
  ...
  const auto instance_text{R"JSON("foo")JSON"};
  std::istringstream stream{instance_text};
  sourcemeta::core::PointerPositionTracker tracker;
  const auto instance{sourcemeta::core::parse_json(stream, std::ref(tracker))};
...
TEST(Output_standard_position, position_values_correct) {
  const auto schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "properties": {
      "foo": { "type": "string" }
  ...
  EXPECT_GT(position.at(3).to_integer(), position.at(1).to_integer());
}
