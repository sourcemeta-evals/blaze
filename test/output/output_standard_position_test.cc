...
TEST(Output_standard_position, array_item_error_with_position) {
  ...
  bool found_error_with_position = false;
  for (const auto &error : result.at("errors").as_array()) {
    if (error.defines("instanceLocation") &&
        error.at("instanceLocation").to_string() == "/1" &&
        error.defines("instancePosition") &&
        error.at("instancePosition").is_array() &&
        error.at("instancePosition").size() == 4) {
      found_error_with_position = true;
      break;
    }
  }
  EXPECT_TRUE(found_error_with_position);
}
