...
TEST(Compiler_output_simple, annotations_drop_on_contains_fail_items) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": {
      "type": "number",
  ...
  const auto not_found_0 = output.annotations().find(
      {sourcemeta::core::to_weak_pointer(sourcemeta::core::to_pointer("/0")),
       ...
       "#/contains/title"});
  EXPECT_TRUE(not_found_0 == output.annotations().end());

  const auto not_found_2 = output.annotations().find(
      {sourcemeta::core::to_weak_pointer(sourcemeta::core::to_pointer("/2")),
       sourcemeta::core::to_weak_pointer(
           sourcemeta::core::to_pointer("/contains/title")),
       "#/contains/title"});
  EXPECT_TRUE(not_found_2 == output.annotations().end());
  EXPECT_TRUE(not_found_2 == output.annotations().end());
}
