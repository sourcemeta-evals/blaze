...
TEST(Compiler_output_simple, contains_title_annotation_failure) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": { 
      "type": "number",
  ...
  // Clean up contains annotations after evaluation is complete
  output.cleanup_contains_annotations();

  // Should only have contains annotation for index 1 (the number 42)
  // Should NOT have title annotations for indices 0 and 2 (failed items)
  EXPECT_ANNOTATION_COUNT(output, 1);
  EXPECT_ANNOTATION_ENTRY(output, "", "/contains", "#/contains", 1);
  EXPECT_ANNOTATION_VALUE(output, "", "/contains", "#/contains", 0,
                          sourcemeta::core::JSON{1});
