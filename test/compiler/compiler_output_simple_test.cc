...
TEST(Compiler_output_simple, contains_annotations_failure_bug) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": { 
      "type": "number",
  ...
  // Clean up contains annotations after validation
  output.cleanup_contains_annotations();


  // The bug: annotations should NOT be retained for failed contains subschemas
  // Only the annotation for the successful match at index 1 should remain
  EXPECT_ANNOTATION_COUNT(output, 2);
