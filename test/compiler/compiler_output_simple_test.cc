...
TEST(Compiler_output_simple, annotations_contains_drops_failing_items_1) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": {
      "type": "number",
  ...
  // plus the contains annotation itself. Annotations for /0 and /2
  // must be dropped because those items failed against contains.
  EXPECT_ANNOTATION_COUNT(output, 2);
...
TEST(Compiler_output_simple, annotations_contains_drops_failing_items_2) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": {
      "type": "number",
  ...
  // All items match, so all annotations should be retained
...
TEST(Compiler_output_simple, annotations_contains_drops_failing_items_3) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": {
      "type": "number",
  ...
  // No items match, so contains itself fails
  EXPECT_FALSE(result);
  EXPECT_ANNOTATION_COUNT(output, 0);
}
