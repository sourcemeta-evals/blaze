...
TEST(Compiler_output_simple, annotations_contains_title_per_item) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": {
      "type": "number",
  ...
  // contains should pass overall
  EXPECT_TRUE(result);

  // Should have 2 annotations: contains at root and title for matching item
  // only
  EXPECT_ANNOTATION_COUNT(output, 2);

  // Contains annotation at root level with matching index
  EXPECT_ANNOTATION_ENTRY(output, "", "/contains", "#/contains", 1);
  EXPECT_ANNOTATION_VALUE(output, "", "/contains", "#/contains", 0,
                          sourcemeta::core::JSON{1});

  // Title annotation only for the matching item at /1
...
TEST(Compiler_output_simple, annotations_contains_title_overall_failure) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": {
      "type": "number",
  ...
  // contains should fail overall, and no annotations should be retained
  EXPECT_FALSE(result);
  EXPECT_ANNOTATION_COUNT(output, 0);
}
