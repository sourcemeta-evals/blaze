...
TEST(Compiler_output_simple, contains_title_annotations_dropped_correctly) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": { 
      "type": "number",
  ...
  // 1. The title annotation for /1 (the number that passed)
  // 2. The contains annotation at root level indicating contains succeeded
  EXPECT_ANNOTATION_COUNT(output, 2);
