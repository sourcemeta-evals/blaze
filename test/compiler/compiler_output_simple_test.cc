...
TEST(Compiler_output_simple, contains_title_annotation_cleanup) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": { 
      "type": "number",
  ...
  // Annotations for failed items at indices 0 and 2 should be dropped
  EXPECT_ANNOTATION_COUNT(output, 2);

  // Contains annotation for the successful match
  EXPECT_ANNOTATION_ENTRY(output, "", "/contains", "#/contains", 1);
  EXPECT_ANNOTATION_VALUE(output, "", "/contains", "#/contains", 0,
                          sourcemeta::core::JSON{1});

  // Title annotation should only exist for the successful item at /1
