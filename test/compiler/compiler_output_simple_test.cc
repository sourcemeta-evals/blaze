...
TEST(Compiler_output_simple, annotations_success_10) {
  ...
  const auto contains_title_instance_0{sourcemeta::core::to_pointer("/0")};
  const auto contains_title_instance_2{sourcemeta::core::to_pointer("/2")};
  const auto contains_title_path{
      sourcemeta::core::to_pointer("/contains/title")};
  EXPECT_FALSE(output.annotations().contains(
      {sourcemeta::core::to_weak_pointer(contains_title_instance_0),
       sourcemeta::core::to_weak_pointer(contains_title_path),
       "#/contains/title"}));
  EXPECT_FALSE(output.annotations().contains(
      {sourcemeta::core::to_weak_pointer(contains_title_instance_2),
       sourcemeta::core::to_weak_pointer(contains_title_path),
       "#/contains/title"}));
}
