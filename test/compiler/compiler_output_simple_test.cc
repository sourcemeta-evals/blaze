...
TEST(Compiler_output_simple, contains_annotation_issue_reproduction) {
  // Schema from the task description
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": { 
      "type": "number",
  ...
  // Instance from the task description
  const sourcemeta::core::JSON instance{
  ...
  EXPECT_TRUE(result); // Should pass because 42 matches

  // After fix, should only have annotation for /1 (the successful item) and the
  // contains count
  EXPECT_ANNOTATION_COUNT(output, 2);
