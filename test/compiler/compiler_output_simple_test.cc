...
TEST(Compiler_output_simple, annotations_contains_with_title_2) {
  ...
  // All items pass, so we should have annotations for all of them
...
TEST(Compiler_output_simple, annotations_contains_with_title_3) {
  ...
  // No items pass, so no annotations should be retained
  EXPECT_ANNOTATION_COUNT(output, 0);
}
...
TEST(Compiler_output_simple, annotations_contains_multiple_matches) {
  ...
  const sourcemeta::core::JSON instance{sourcemeta::core::parse_json(
      R"JSON([ "foo", 42, "bar", 100, true ])JSON")};
  ...
  // Should have annotations only for items that passed (indices 1 and 3)
