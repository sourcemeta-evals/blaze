...
TEST(Compiler_output_simple, annotations_contains_with_title_1) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": {
      "type": "number",
  ...
  // Only the item at index 1 (42) passes the contains subschema,
  // so only its title annotation should be retained
  EXPECT_ANNOTATION_COUNT(output, 2);
...
TEST(Compiler_output_simple, annotations_contains_with_title_2) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": {
      "type": "number",
  ...
  // All items pass the contains subschema, so all title annotations
  // should be retained
...
TEST(Compiler_output_simple, annotations_contains_with_title_3) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": {
      "type": "number",
  ...
  const sourcemeta::core::JSON instance{
      sourcemeta::core::parse_json(R"JSON([ "a", 1, "b", 2, "c" ])JSON")};
  ...
  // Only items at indices 1 and 3 pass the contains subschema,
  // so only their title annotations should be retained
