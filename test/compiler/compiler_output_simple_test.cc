...
TEST(Compiler_output_simple, annotations_contains_drops_failed_item_titles) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": {
      "type": "number",
  ...
  const auto contains_title_path{sourcemeta::core::to_weak_pointer(
  ...
  EXPECT_FALSE(output.annotations().contains(
      {sourcemeta::core::to_weak_pointer(sourcemeta::core::to_pointer("/0")),
       contains_title_path, "#/contains/title"}));
  EXPECT_FALSE(output.annotations().contains(
