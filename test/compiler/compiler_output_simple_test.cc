...
TEST(Compiler_output_simple, contains_annotation_dropping_bug) {
  ...
  // The bug: annotations should only exist for /1 (the number 42),
  // not for /0 ("foo") and /2 (true) since those failed the contains subschema
  EXPECT_ANNOTATION_COUNT(output, 2);

  // Contains annotation at array level with matching index
  EXPECT_ANNOTATION_ENTRY(output, "", "/contains", "#/contains", 1);
  EXPECT_ANNOTATION_VALUE(output, "", "/contains", "#/contains", 0,
                          sourcemeta::core::JSON{1});

  // Title annotation only for the matching item
