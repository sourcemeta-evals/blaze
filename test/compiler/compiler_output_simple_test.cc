...
TEST(SimpleOutput, contains_annotations_partial_failure) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": { 
      "type": "number",
  ...
  // Clean up annotations based on evaluation failures
  output.cleanup_annotations();

  // Should only have annotations for the root contains and the successful item
  // at /1
