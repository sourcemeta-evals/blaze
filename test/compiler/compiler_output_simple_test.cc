...
TEST(Compiler_output_simple, contains_title_annotation_dropping) {
  ...
  // 1. Contains annotation at root indicating which indices matched (value: 1)
  // 2. Title annotation only for the matching element at /1 (value: "Test")
  EXPECT_ANNOTATION_COUNT(output, 2);

  // Contains annotation at root level
  EXPECT_ANNOTATION_ENTRY(output, "", "/contains", "#/contains", 1);
  EXPECT_ANNOTATION_VALUE(output, "", "/contains", "#/contains", 0,
                          sourcemeta::core::JSON{1});

  // Title annotation only for the matching element at /1
