...
TEST(Compiler_output_simple, contains_title_annotation_bug_reproduction) {
  ...
  // Should only have 2 annotations: contains annotation and title for
  // successful item
  EXPECT_ANNOTATION_COUNT(output, 2);

  // Should have contains annotation showing successful match at index 1
  EXPECT_ANNOTATION_ENTRY(output, "", "/contains", "#/contains", 1);
  EXPECT_ANNOTATION_VALUE(output, "", "/contains", "#/contains", 0,
                          sourcemeta::core::JSON{1});

  // Should have title annotation only for the successful item at /1
...
TEST(Compiler_output_simple, contains_title_annotation_correct_behavior) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": { 
      "type": "number",
  ...
  // Should have both contains and title annotations
  EXPECT_ANNOTATION_COUNT(output, 2);
