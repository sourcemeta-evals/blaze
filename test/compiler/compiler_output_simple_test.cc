...
TEST(Compiler_output_simple, contains_annotations_cleanup) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": { 
      "type": "number",
  ...
  // Should only have 2 annotations: contains count and title for successful
  // item
  EXPECT_ANNOTATION_COUNT(output, 2);

  // Contains annotation indicating 1 match at index 1
  EXPECT_ANNOTATION_ENTRY(output, "", "/contains", "#/contains", 1);
  EXPECT_ANNOTATION_VALUE(output, "", "/contains", "#/contains", 0,
                          sourcemeta::core::JSON{1});

  // Title annotation only for the successful item at /1 (not for /0 or /2)
