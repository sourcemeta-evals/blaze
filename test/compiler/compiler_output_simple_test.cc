...
TEST(Compiler_output_simple, contains_drops_failed_item_annotations) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": { 
      "type": "number",
  ...
  // Should have 2 annotation entries:
  // 1. The contains annotation at root level (instance location "")
  // 2. The title annotation for /1 only (the item that matched)
  EXPECT_ANNOTATION_COUNT(output, 2);

  // Check contains annotation at root
  EXPECT_ANNOTATION_ENTRY(output, "", "/contains", "#/contains", 1);
  EXPECT_ANNOTATION_VALUE(output, "", "/contains", "#/contains", 0,
                          sourcemeta::core::JSON{1});
