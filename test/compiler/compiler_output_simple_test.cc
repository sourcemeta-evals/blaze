...
TEST(Compiler_output_simple, contains_annotation_filtering) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": { 
      "type": "number",
  ...
  // Should have 2 annotations: 1 contains annotation + 1 title annotation for
  // the matching item
  EXPECT_ANNOTATION_COUNT(output, 2);

  // Contains annotation at root level with index 1 (the number 42)
  EXPECT_ANNOTATION_ENTRY(output, "", "/contains", "#/contains", 1);
  EXPECT_ANNOTATION_VALUE(output, "", "/contains", "#/contains", 0,
                          sourcemeta::core::JSON{1});

  // Title annotation only for the item that passed contains validation (index
  // 1)
