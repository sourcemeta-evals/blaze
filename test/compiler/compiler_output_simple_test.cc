...
TEST(Compiler_output_simple,
     annotations_success_contains_drops_failed_items_1) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": {
      "type": "number",
  ...
  // The title annotation must only be retained for the item that
  // actually matched contains (index 1), not for items 0 and 2
  EXPECT_ANNOTATION_COUNT(output, 2);
...
TEST(Compiler_output_simple,
     annotations_success_contains_drops_failed_items_2) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": {
      "type": "number",
  ...
  const sourcemeta::core::JSON instance{
      sourcemeta::core::parse_json(R"JSON([ 1, "foo", 2 ])JSON")};
  ...
  // Items 0 and 2 match, item 1 does not
...
TEST(Compiler_output_simple,
     annotations_success_contains_drops_failed_items_3) {
  const sourcemeta::core::JSON schema{sourcemeta::core::parse_json(R"JSON({
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "contains": {
      "type": "number",
  ...
  // Both title and description annotations must only be retained
  // for item 1, not for items 0 and 2
  EXPECT_ANNOTATION_COUNT(output, 3);
  ...
  EXPECT_ANNOTATION_ENTRY(output, "/1", "/contains/description",
                          "#/contains/description", 1);
  EXPECT_ANNOTATION_VALUE(output, "/1", "/contains/description",
                          "#/contains/description", 0,
                          sourcemeta::core::JSON{"A number"});
}
