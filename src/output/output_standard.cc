...
auto standard(const sourcemeta::core::PointerPositionTracker &instanceTracker,
              Evaluator &evaluator, const Template &schema,
              const sourcemeta::core::JSON &instance,
              const StandardOutput format) -> sourcemeta::core::JSON {
  if (format == StandardOutput::Flag) {
    auto result{sourcemeta::core::JSON::make_object()};
    ...
    if (valid) {
      ...
      for (const auto &annotation : output.annotations()) {
        ...
        sourcemeta::core::Pointer ptr;
        for (const auto &token : annotation.first.instance_location) {
          if (token.is_property()) {
            ptr.push_back(token.to_property());
          } else {
            ptr.push_back(token.to_index());
          }
        }
        const auto position{instanceTracker.get(ptr)};
        if (position.has_value()) {
          auto pos_array{sourcemeta::core::JSON::make_array()};
          pos_array.push_back(sourcemeta::core::JSON{
              static_cast<std::size_t>(std::get<0>(position.value()))});
          pos_array.push_back(sourcemeta::core::JSON{
              static_cast<std::size_t>(std::get<1>(position.value()))});
          pos_array.push_back(sourcemeta::core::JSON{
              static_cast<std::size_t>(std::get<2>(position.value()))});
          pos_array.push_back(sourcemeta::core::JSON{
              static_cast<std::size_t>(std::get<3>(position.value()))});
          unit.assign("instancePosition", std::move(pos_array));
        }
