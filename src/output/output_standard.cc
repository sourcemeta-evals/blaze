...
auto set_instance_position(
  ...
  const auto position{
      tracker->get(sourcemeta::core::to_pointer(instance_location))};
...
auto standard_impl(sourcemeta::blaze::Evaluator &evaluator,
  ...
  if (format == sourcemeta::blaze::StandardOutput::Flag) {
    ...
    if (valid) {
      ...
      for (const auto &entry : output) {
        ...
        set_instance_position(unit, entry.instance_location, tracker);
...
auto standard(Evaluator &evaluator, const Template &schema,
              ...
              const sourcemeta::core::PointerPositionTracker &tracker)
    -> sourcemeta::core::JSON {
  return standard_impl(evaluator, schema, instance, format, &tracker);
}
