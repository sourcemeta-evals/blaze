...
namespace {
auto weak_pointer_to_pointer(const sourcemeta::core::WeakPointer &weak_ptr)
    -> sourcemeta::core::PointerPositionTracker::Pointer {
  sourcemeta::core::PointerPositionTracker::Pointer result;
  for (const auto &token : weak_ptr) {
  ...
  if (format == StandardOutput::Flag) {
    ...
    if (valid) {
      ...
      for (const auto &annotation : output.annotations()) {
        ...
        const auto pointer{
            weak_pointer_to_pointer(annotation.first.instance_location)};
      ...
      for (const auto &entry : output) {
        ...
        const auto pointer{weak_pointer_to_pointer(entry.instance_location)};
        const auto position{tracker.get(pointer)};
        if (position.has_value()) {
          auto position_array{sourcemeta::core::JSON::make_array()};
          position_array.push_back(
