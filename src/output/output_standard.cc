  ...
  if (format == StandardOutput::Flag) {
    ...
    if (valid) {
      ...
      for (const auto &annotation : output.annotations()) {
        ...
        // Add instance position if available
        // Convert WeakPointer to Pointer
        sourcemeta::core::Pointer location_pointer;
        for (const auto &token : annotation.first.instance_location) {
          if (token.is_property()) {
            location_pointer.push_back(token.to_property());
          } else {
            location_pointer.push_back(token.to_index());
          }
        }
        const auto position{positions.get(location_pointer)};
