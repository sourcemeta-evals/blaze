...
auto augment_instance_position(
  ...
  value.push_back(sourcemeta::core::JSON{std::get<0>(position.value())});
  value.push_back(sourcemeta::core::JSON{std::get<1>(position.value())});
  value.push_back(sourcemeta::core::JSON{std::get<2>(position.value())});
  value.push_back(sourcemeta::core::JSON{std::get<3>(position.value())});
  unit.assign("instancePosition", std::move(value));
}
    ...
    if (valid) {
      ...
      for (const auto &annotation : output.annotations()) {
        ...
        unit.assign(
            "instanceLocation",
            sourcemeta::core::to_json(annotation.first.instance_location));
        augment_instance_position(unit, annotation.first.instance_location,
                                  positions);
        unit.assign("annotation", sourcemeta::core::to_json(annotation.second));
        annotations.push_back(std::move(unit));
      }
      ...
      for (const auto &entry : output) {
        ...
        unit.assign("instanceLocation",
                    sourcemeta::core::to_json(entry.instance_location));
        augment_instance_position(unit, entry.instance_location, positions);
        unit.assign("error", sourcemeta::core::JSON{entry.message});
        errors.push_back(std::move(unit));
      }
