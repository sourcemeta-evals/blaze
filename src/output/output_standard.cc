...
static auto instance_position_array(
    const sourcemeta::core::PointerPositionTracker::Position &position)
    -> sourcemeta::core::JSON {
  auto result{sourcemeta::core::JSON::make_array()};
  result.push_back(
      sourcemeta::core::JSON{static_cast<std::int64_t>(std::get<0>(position))});
  result.push_back(
      sourcemeta::core::JSON{static_cast<std::int64_t>(std::get<1>(position))});
  result.push_back(
      sourcemeta::core::JSON{static_cast<std::int64_t>(std::get<2>(position))});
  result.push_back(
      sourcemeta::core::JSON{static_cast<std::int64_t>(std::get<3>(position))});
  return result;
}
      ...
      for (const auto &annotation : output.annotations()) {
        ...
        if (position.has_value()) {
          unit.assign("instancePosition",
                      instance_position_array(position.value()));
        }
