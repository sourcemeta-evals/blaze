...
auto SimpleOutput::operator()(
  ...
  // Drop annotations for the specific (evaluate_path, instance_location)
  // combination when a step fails. This is important for `contains` where
  // individual array items may fail but the overall `contains` passes.
  // Annotations from failing items should not be retained.
