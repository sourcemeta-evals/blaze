...
auto SimpleOutput::operator()(
  ...
  // Check if we're inside a contains evaluation that should not mask errors
  const bool inside_contains_no_mask = std::any_of(
      this->mask.cbegin(), this->mask.cend(),
      [&evaluate_path](const auto &entry) {
        return evaluate_path.starts_with(entry.first) && !entry.second;
      });

  if (inside_contains_no_mask) {
    // Track this specific failure for contains
    // We need to find the contains evaluate path and track the instance
    // location
  ...
  if (type == EvaluationType::Post && !this->annotations_.empty()) {
    for (auto iterator = this->annotations_.begin();
         iterator != this->annotations_.end();) {
      // Check if the annotation's evaluate path starts with a failing path
