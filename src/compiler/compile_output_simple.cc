...
auto SimpleOutput::operator()(
  ...
  // Special handling for contains: drop annotations for failed array elements
  if (type == EvaluationType::Post && !result && !this->annotations_.empty()) {
    // Check if this is a contains subschema evaluation failure
    bool is_contains_subschema = false;
    for (const auto& path_element : evaluate_path) {
      if (path_element.is_property() && path_element.to_property() == "contains") {
    ...
    if (is_contains_subschema && !instance_location.empty()) {
      // Drop all annotations for this specific instance location within contains
      for (auto iterator = this->annotations_.begin();
           iterator != this->annotations_.end();) {
        if (iterator->first.instance_location == instance_location) {
          ...
          for (const auto& path_element : iterator->first.evaluate_path) {
            if (path_element.is_property() && path_element.to_property() == "contains") {
