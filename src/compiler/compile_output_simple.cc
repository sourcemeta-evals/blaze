...
auto SimpleOutput::operator()(
  ...
  // Check if we're under a masked contains (value false means contains)
  ...
  if (masked_contains != this->mask.cend()) {
    // Before returning early for masked contains failures, drop annotations
    // for this specific instance location under the contains subschema
