...
auto SimpleOutput::operator()(
  ...
  // Erase annotations for failed paths, considering both evaluate path and
  // instance location. This must happen before the mask check below, as
  // keywords like `contains` mask failures but still need to drop annotations
  // for items that failed against the subschema.
