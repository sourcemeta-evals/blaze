...
auto SimpleOutput::operator()(
  ...
  // Drop annotations for failed evaluations. We need to do this BEFORE
  // the contains early return below, as annotations from failed contains
  // items should be dropped. The check uses both evaluate_path and
  // instance_location because in JSON Schema, a trace of evaluation is
  // only uniquely identified by the combination of both.
